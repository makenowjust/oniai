...
/// [`MULTI_CASE_FOLDS`] tables (generated by `build.rs`) instead of
/// scanning all 1.1 M Unicode codepoints at runtime.  Each table has
/// ≈ 1 400 entries — a >700× reduction over the full-codepoint scan.
///
...
// Static tables generated by build.rs from the unicode-casefold crate.
include!(concat!(env!("OUT_DIR"), "/casefold_data.rs"));
...
/// simple-case-fold to a value within the charset.  This is the compile-time
/// cost paid at `Regex::new()`.  The resulting trie replaces charset matching
/// in the VM hot path with no `case_fold()` or Unicode-property calls.
///
